...
function Main() {
	...
	const fetchMovies: () => Promise<MovieList> = useCallback(async () => {
		try {
			const res: TypeCommon = await dispatch(movieListThunk({ status, limit, order, cursor }));
			return res.payload || [];
		...
		<div className="grid grid-cols-12 gap-y-10 gap-x-6">
			<div className="col-span-12">
				<div className="flex flex-col md:h-10 gap-y-3 md:items-center md:flex-row">
					<div className="text-base font-medium group-[.mode--light]:text-white">Films</div>
					...
					<div className="flex flex-col box box--stacked">
						<div className="flex flex-col p-5 sm:items-center sm:flex-row gap-y-2">
							<div>
								<div className="relative">
									...
									/>
									<FormInput type="text" placeholder="Search film..." className="pl-9 sm:w-64 rounded-[0.5rem]" />
								</div>
							</div>
							<div className="flex flex-col sm:flex-row gap-x-3 gap-y-2 sm:ml-auto">
								<div className="flex flex-col mt-2 sm:flex-row">
									<FormCheck className="mr-2">
										...
										/>
										<FormCheck.Label htmlFor="radio-switch-4">Oldest</FormCheck.Label>
									</FormCheck>
									<FormCheck className="mt-2 mr-2 sm:mt-0">
										<FormCheck.Input
											...
											onChange={(e) => setOrder(e.target.value)}
										/>
										<FormCheck.Label htmlFor="radio-switch-5">Newest</FormCheck.Label>
									</FormCheck>
								</div>
								<Popover className="inline-block">
									{({ close }) => (
										<>
											<Popover.Button as={Button} variant="outline-secondary" className="w-full sm:w-auto">
								...
								{movies?.map((movie, key) => (
									...
									>
										<div className="overflow-hidden rounded-lg h-[30rem] image-fit before:block before:absolute before:w-full before:h-full before:top-0 before:left-0 before:z-10 before:bg-gradient-to-t before:from-slate-900/90 before:to-black/20">
											<img className="rounded-md" src={movie.thumbnail} alt={movie.name} />
											<span
											...
											<div className="absolute bottom-0 z-10 w-full px-5 pb-6 text-white">
												<Link className="block text-lg font-medium truncate" to={`/film/${movie.id}`}>
													{movie.name}
												</Link>
												{movie.movie_length !== 0 ? (
													<span className="mt-3 text-xs text-white/80">{movie.movie_length} minutes</span>
												) : (
										...
										<div className="pt-5">
													...
													<div className="ml-auto">
														<div className="flex items-center text-xs font-medium rounded-md text-success bg-success/10 border border-success/10 px-1.5 py-px">
															<span className="-mt-px">
																{movie.codes.map((code) => code.replace("CODE_", "")).join(", ")}
															</span>
														</div>
													</div>
												</div>
											...
											<div className="flex items-center">
												<Link className="flex items-center mr-auto text-primary" to={`/film/${movie.id}`}>
													<Lucide icon="KanbanSquare" className="w-4 h-4 stroke-[1.3] mr-1.5" /> View
												</Link>
												<Link className="flex mr-5 text-primary" to={`/update-film/${movie.id}`}>
													<Lucide icon="CheckSquare" className="w-4 h-4 stroke-[1.3] mr-1.5" />
													Edit
												</Link>
												<Preview>
													<button
													...
													>
														<Lucide icon="Trash2" className="w-4 h-4 stroke-[1.3] mr-1.5" /> Delete
													</button>
													<Dialog
						...
						<div className="flex flex-col-reverse flex-wrap items-center p-5 flex-reverse gap-y-2 sm:flex-row">
							<Pagination className="flex-1 w-full mr-auto sm:w-auto">
								<Pagination.Link>
									<button onClick={() => setCursor(page?.prevCursor || "")} disabled={!page?.prevCursor || page.prevCursor === ""}>
										<Lucide icon="ChevronLeft" size={100} color={!page?.prevCursor || page.prevCursor === "" ? "grey" : "red"} />
									</button>
								</Pagination.Link>
								<Pagination.Link>
									<button onClick={() => setCursor(page?.nextCursor || "")} disabled={!page?.nextCursor || page.nextCursor === ""}>
										<Lucide icon="ChevronRight" size={100} color={!page?.nextCursor || page.nextCursor === "" ? "grey" : "red"} />
									</button>
								</Pagination.Link>
							</Pagination>
							<FormSelect
